...
To have the proxy check the authenticity of the gateway device, you can register the gateway's public key in `server.diag`:

You can replace the `"peer_cred": { "unauthenticated": true }` section by `"peer_cred": {14:<identity>}` and filling `<identity>` with the identity generated by the gateway firmware, it is an INFO log in the startup of the `../gateway` nrf9151 firmware (this means you have to run the `../gateway` firmware once before deploying the CoAP proxy), for example:

```log
[INFO ] CoAP server identity: {8:{1:{1:2, 2:h'', -1:1, -2:h'dfb434e26cbd1bf8adbbf8075acaf6ff93fd79e774fce9a25ab55f72c6f4bb2f'}}} (ariel_os_coap ariel-os-coap/src/stored.rs:116)
```

Here the identity would be `{8:{1:{1:2, 2:h'', -1:1, -2:h'dfb434e26cbd1bf8adbbf8075acaf6ff93fd79e774fce9a25ab55f72c6f4bb2f'}}}`, the `"peer_cred"` section would look like:

```cbordiag
"peer_cred": {14:{8:{1:{1:2, 2:h'', -1:1, -2:h'dfb434e26cbd1bf8adbbf8075acaf6ff93fd79e774fce9a25ab55f72c6f4bb2f'}}}},
```
